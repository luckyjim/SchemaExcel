...
class MasterParameterToJson(object):
    ...
    def get_schema_json(self, component):
        """
        Create schema json associated to component

        :param component: name of sheet where component parameters are defined
        :type component: str
        ...
        self.json_dict_prefix = {
            ...
            "description": f"Parameters for {component.upper()}",
            "type": "object",
            "properties": {},
            "required": [],
        }
    ...
    def _get_sheet_component(self, component):
        """get component ie worksheet description from Excel master parameter file

        :param component: name of component ie name of worksheet
        :type component: string
        :raises TypeError: error if extension of file is not .xlsx
        :raises FileNotFoundError: error if file does not exist
        :raises KeyError: error if component ie sheet does not exists in booksheet
        :return: component description extracted from sheet
        :rtype: instance of pyexcel Sheet class
        """
        self.json_file = f"{component.lower()}_schema.json"
        if self.xlsx_filename.rsplit(".")[-1] != "xlsx":
            logger.error(
                f'File {self.xlsx_filename} have not the good type. It must be ".xlsx"'
            )
            raise TypeError(
                f'File {self.xlsx_filename} have not the good type. It must be ".xlsx"'
            )
            return
        if not osp.exists(self.xlsx_filename):
            logger.error(f"File {self.xlsx_filename} does not exist")
            raise FileNotFoundError(f"File {self.xlsx_filename} does not exist")
            return

        xlsx_file_path = self.xlsx_filename
        ...
        if component in book.sheet_names():
            ...
            logger.info(
                f"init MasterParameterToJson with {self.xlsx_filename}/{component}"
            )
            return sheet
        else:
            logger.error(
                f"Component {component.upper()} does not exist in {xlsx_file_path}"
            )
            raise KeyError(
                f"Component {component.upper()} does not exist in {xlsx_file_path}"
            )
    ...
    def _get_cell_value(self, rowname, colname):
        """Get cell value of component sheet with row name and column name
        ...
        :rtype: string
        """
        if rowname not in self.component.rownames:
            raise KeyError(
                f"Parameter '{rowname}' does not exist in {self.component.rownames}"
            )
            return
...
#         """
#         Create file schema json for all components in ECPI master file parameters
#         """
